...
const SecurityManagement = () => {
  ...
  const fetchSecurityGuards = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/security${branchParam}`);
      if (response.ok) {
      ...
      console.error("Failed to fetch security guards:", err);
      toast.error("Network error while syncing directory.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (user) fetchSecurityGuards();
  }, [user]);

  const handleStatusUpdate = async (securityId, newStatus) => {
    try {
      const response = await fetch(`${API_URL}/api/security/${securityId}/status`, {
        method: 'PUT',
      ...
      toast.success(`Security Guard marked as ${newStatus}`);
      fetchSecurityGuards();
    } catch (e) { 
  ...
  const filteredSecurityGuards = securityGuards.filter(emp => 
    ...
    <div className="container-fluid mt-4">
      <div className="card p-4 shadow-sm border-0 rounded-3">
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div>
            <h3 className="fw-bold text-primary mb-1"><ShieldCheck className="me-2" /> Security Directory</h3>
            <p className="text-muted mb-0">Manage and review security guard access and registrations.</p>
          </div>
        ...
        <ul className="nav nav-tabs mb-4">
          <li className="nav-item">
            ...
            >
              Approved (Active)
              <span className="badge bg-success ms-2">{securityGuards.filter(e => e.status === 'APPROVED').length}</span>
            </button>
          </li>
          <li className="nav-item">
            <button 
            ...
            >
              Pending Approval
              <span className="badge bg-warning text-dark ms-2">{securityGuards.filter(e => e.status === 'PENDING').length}</span>
            </button>
          </li>
          <li className="nav-item">
            <button 
            ...
            >
              Rejected
              <span className="badge bg-danger ms-2">{securityGuards.filter(e => e.status === 'REJECTED').length}</span>
            </button>
          </li>
        </ul>
        ...
        <div className="table-responsive bg-white rounded border">
          <table className="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              ) : filteredSecurityGuards.length === 0 ? (
                <tr><td colSpan="5" className="text-center py-5 text-muted">No security guards found in this category.</td></tr>
              ) : (
                filteredSecurityGuards.map((emp) => (
                  <tr key={emp.id}>
